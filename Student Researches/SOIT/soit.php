...
<h1 class="text-center">RESEARCHES 2015-2021&nbsp;</h1>
  	    <h3 class="text-center">SCHOOL OF INDUSTRIAL TECHNOLOGY (SOIT)</h3>
		   <div class="col-1 px-1" style="margin-bottom: 2%"><a href="../../studentResearches.php"><button class="btn-secondary py-1 rounded px-4">Return</button></a></div>

  	   <div class="">
        <label>Filter By Year:</label>
        <div class="row px-2">
         <div class="col-2 px-1">
       <form action="soit.php" method="POST">
           <select class="form-control" name="searching" required="">
            <option></option>
            <?php 
            $sel_yr = mysqli_query($connection,"SELECT  DISTINCT yr FROM research WHERE research_category= 1 AND dept='SOIT' ORDER BY yr DESC ") or die(mysqli_error($connection));
        ...
        <div class="col-1 px-1" style="margin-left: 65%"><a href="soit.php"><button class="btn-primary py-1 rounded px-3">Refresh</button></a></div>
     </div>
 </div><br><br>
 <form action="../../nextpage.php" method="POST">
   <div class="row">
    ...
    if(isset($_POST['search'])){
      ...
      $sel_sbm = mysqli_query($connection,"SELECT DISTINCT yr,dept,research_category FROM research WHERE dept='SOIT' AND research_category = 1 AND yr='$searching'")or die(mysqli_error($connection));
    }else{
      $sel_sbm = mysqli_query($connection,"SELECT DISTINCT yr,dept,research_category FROM research WHERE dept='SOIT' AND research_category = 1 ORDER BY yr DESC")or die(mysqli_error($connection));
    }
      while($res = mysqli_fetch_array($sel_sbm)){ 
        ?>
          ...
          <div class="col-4 px- py-1 mb-2">
            ...
            <p class="card-text">This page contains researches oF SOIT year <?php echo $res['yr'] ?>.</p>
